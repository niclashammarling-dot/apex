...
- PATTERN (2026-05-27): L1 gate selection systematically compresses input distributions for momentum and volume signals. Both momentum_weight_analysis.py and volume_divisor_analysis.py (N=184 each) returned fully flat sweep tables — all parameter values produce identical trade selection. Mechanism: L1 selects momentum names where both price momentum and RSI co-move positively (momentum split invariant) and where institutional volume is steady rather than spiked (volume divisor invariant). Aggregator weights for these two signals (0.25 momentum + 0.20 volume = 0.45 total) are allocated to signals with near-zero discriminative spread in the post-L1 universe. The remaining 0.55 weight (ev_norm, trend_score, rs_score) carries the actual ranking work. Aggregator weight calibration is the open question.
