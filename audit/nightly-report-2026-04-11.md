...
| check_num check_name | ⚠ | SEV | file:line | one-line description |
|----------------------|---|-----|------------|----------------------|
| 7 Demo/live gate runner parity | ⚠ | WARNING | gate_runner.py:3 | Demo runner uses `get_ticker_gate_fails` while live uses `get_live_ticker_gate_fails`, which may lead to inconsistent gate fail history retrieval. |
...
| 8c Inconsistent return types | ⚠ | WARNING | gate_runner.py:43 | Function `_gate_result` returns `None` for `lock_leading_checks` without caller guards. |
