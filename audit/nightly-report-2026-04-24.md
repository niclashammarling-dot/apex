...
| 8 Inconsistent return types | ⚠ | WARNING | gate_runner.py:15 | Function returns `None` for `rotation_regime_conditioned` and `rotation_regime_sample_size` without caller guard. |
| 8 Inconsistent return types | ⚠ | WARNING | gate_runner.py:38 | Function returns `None` for `ticker_gate_history` without caller guard. |
