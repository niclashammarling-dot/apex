...
For `position_size_pct`: if your decision is BUY, set this to risk_limits.max_position_size unless a specific portfolio risk factor (high drawdown, near position limit, sector near capacity) warrants reduction. Do NOT factor regime_bayes_allocation, regime_bayes_posterior, regime_bayes_adjusted_score, regime_bayes_qualified, regime_bayes_rank, or any Bayesian allocation field into your size — the system applies a separate mechanical Bayesian multiplier to your output after this gate. Your sizing is pure portfolio risk management; allocation weighting is handled downstream and is not your concern.

Two regime fields form one statement. `market_leader` is the snapshot answer (highest raw sector score). `bayes_margin` is the trust-qualifier: 0 means conviction confirms the leader; above the signal threshold (~0.10) means the conviction layer has meaningfully higher belief in a different sector — in that case `bayes_conviction_leader` names it. Near-zero margin means treat the snapshot as trustworthy; high margin means treat the snapshot with caution and weigh the conviction leader more heavily. Do not manufacture a third signal from these two; they are one statement: state / confidence.
