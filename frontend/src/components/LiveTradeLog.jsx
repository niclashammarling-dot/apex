...
export default function LiveTradeLog({ trades = [] }) {
  ...
  const sorted = useMemo(() => {
    ...
    const closed = trades.filter(t =>  t.outcome).sort((a, b) => {
      const av = a[sort.col] ?? "";
      const bv = b[sort.col] ?? "";
  ...
  const open   = trades.filter(t => !t.outcome).length;
    ...
    <div className="card" style={{ marginTop: 16 }}>
      ...
      {trades.length === 0 ? (
        <div className="card-body" style={{ color: "var(--text-3)", fontSize: 13, textAlign: "center", padding: "28px 0" }}>
          Trades will appear here once live bracket orders are placed and filled.
        </div>
      ) : (
