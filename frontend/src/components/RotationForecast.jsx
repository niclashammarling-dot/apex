...
function ProbBar({ probability }) {
  ...
  const color = pct >= 40 ? "#00d48c" : pct >= 25 ? "#6aa0f0" : "#888";
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <div style={{ background: "#2a2a28", borderRadius: 3, height: 4, width: 60 }}>
        <div style={{ width: `${pct}%`, height: "100%", background: color, borderRadius: 3 }} />
      </div>
      <span style={{ fontFamily: "'Inconsolata', monospace", fontSize: 11, color, fontWeight: 600 }}>
        {pct}%
      </span>
    </div>
  );
...
function TickerList({ sectorTickers, accentColor }) {
  if (!sectorTickers.length) return (
    <div style={{ fontFamily: "'Inconsolata', monospace", fontSize: 10, color: "var(--text-3)", padding: "4px 2px" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
      {sectorTickers.map((t, i) => {
        ...
        const barColor = pct >= 60 ? "#00d48c" : pct >= 40 ? "#6aa0f0" : "#404040";
        return (
          <div key={t.ticker} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              ...
              <div style={{ background: "#2a2a28", borderRadius: 3, height: 3, width: 40 }}>
...
export default function RotationForecast() {
  ...
  if (!data?.available) return (
    <div style={{ fontFamily: "'Inconsolata', monospace", fontSize: 11, color: "var(--text-3)", padding: "8px 0" }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
        {predecessors.map((p, i) => (
          <React.Fragment key={p}>
            <Pill color={i === 0 ? "#343434" : "#404040"}
                  bg={i === 0 ? "#242422" : undefined}
                  border={i === 0 ? "#343434" : undefined}>
              {p}
            </Pill>
        ...
        <span style={{ color: "var(--text-3)", fontSize: 14 }}>→</span>
        <span style={{ fontFamily: "'Inconsolata', monospace", fontSize: 11, color: "var(--text-3)" }}>?</span>
        {ct && (
      ...
      {/* Likely next */}
      <div>
        <div style={{ fontFamily: "'Inconsolata', monospace", fontSize: 10, color: "var(--text-3)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {data.likely_next.map(({ sector, probability, historical_prob, sector_score, sector_signal }, i) => {
              ...
              const pct        = Math.round(probability * 100);
              const probColor  = pct >= 40 ? "#00d48c" : pct >= 25 ? "#6aa0f0" : "#888";
              const rankColors = ["#00d48c", "#6aa0f0", "#7ab8ff"];

              return (
                <div key={sector} style={{
                  background: isWatching ? "#faff6908" : "#1e1e1c",
                  border: `1px solid ${isTop3 ? probColor + "44" : isWatching ? "#00d48c33" : "#2a2a28"}`,
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2 }}>
                        ...
                        {historical_prob != null && (
                          <span style={{ fontFamily: "'Inconsolata', monospace", fontSize: 9, color: "var(--text-3)", opacity: 0.6 }}>
                  ...
                  {isOpen && (
                    <div style={{
                      borderTop: `1px solid ${isTop3 ? probColor + "22" : isWatching ? "#00d48c22" : "#2a2a28"}`,
